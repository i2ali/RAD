...
#include "Stdafx.h"
#include "GdiSlicerOutOfMemory.h"
...
bool GdiSlicerOutOfMemory::ClassSetup(void)
{
...
bool GdiSlicerOutOfMemory::ClassCleanup(void)
{
    return true;
}


bool GdiSlicerOutOfMemory::MethodSetup(void)
{
...
bool GdiSlicerOutOfMemory::MethodCleanup(void)
{
...
void GdiSlicerOutOfMemory::CapturePrimaryDisplay(void)
{
    ...
    // Non-stereo
    {
        ...
        VERIFY_SUCCEEDED(this->mSlicer->StartCapture(topLeft, 100000, 0, RADv2::Channel::Left, &numberOfFramesThatCanBeCaptured));
        ...
        HRESULT hr = this->mSlicer->StartCapture(topLeft, 100000, 0, RADv2::Channel::Left, &numberOfFramesThatCanBeCapturedWithLimitedMemory);
    ...
    // Stereo
    // Windows 8 Bugs #877820
    // TEST - Don't need to test multiple out of memory cases for GDI slicer.
    /*
    {
        UINT numberOfFramesThatCanBeCaptured = 0;
        VERIFY_SUCCEEDED(this->mSlicer->StartCapture(topLeft, 100000, 0, RADv2::Channel::LeftAndRight, &numberOfFramesThatCanBeCaptured));
        Log::Comment(String().Format(L"Number of frames that can be captured: %d", numberOfFramesThatCanBeCaptured));
        ...
        // Now try slicing with limited memory
        UINT numberOfFramesThatCanBeCapturedWithLimitedMemory = 0;
        VERIFY_SUCCEEDED(this->mSlicer->StartCapture(topLeft, 100000, 0, RADv2::Channel::LeftAndRight, &numberOfFramesThatCanBeCapturedWithLimitedMemory));
        Log::Comment(String().Format(L"Number of frames that can be captured: %d", numberOfFramesThatCanBeCapturedWithLimitedMemory));
