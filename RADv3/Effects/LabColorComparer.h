...
//
//  Description:
//      Defines an effect that compares a single LAB color against a LAB
//      formatted image.
//
//------------------------------------------------------------------------------
...
class LabColorComparer : public ID2D1EffectImpl, public ID2D1DrawTransform
{
// Constructor
public:
    LabColorComparer(void);


// Destructor
public:
    virtual ~LabColorComparer(void);
...
// Properties
public:
    HRESULT SetL(FLOAT argL);

    FLOAT GetL(void) const;

    HRESULT SetA(FLOAT argA);

    FLOAT GetA(void) const;

    HRESULT SetB(FLOAT argB);
