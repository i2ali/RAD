...
D2DContext::D2DContext(ComputeShaderModel argComputeShaderModelRequired)
    : mComputeShaderModelRequired(argComputeShaderModelRequired),
      mDxgiFactory(nullptr),
      mD2DFactory(nullptr),
      mD2DDevice(nullptr),
      mD2DDeviceContext(nullptr),
      mLowestFeatureLevelIndex(0),
      mHighestFeatureLevelIndex(0),
      mRefCount(0)
{
}


HRESULT D2DContext::Initialize(void)
{
...
ULONG STDMETHODCALLTYPE D2DContext::AddRef(void)
{
...
ULONG STDMETHODCALLTYPE D2DContext::Release(void)
{
...
HRESULT STDMETHODCALLTYPE D2DContext::QueryInterface(REFIID riid, LPVOID* ppvObj)
{
    if (nullptr == ppvObj)
    {
        return E_INVALIDARG;
    ...
    else if (riid == __uuidof(ID2DContext))
    {
        *ppvObj = static_cast<ID2DContext*>(this);
        this->AddRef();
        return NOERROR;
    }
    else
...
HRESULT D2DContext::GetD2DDevice(ID2D1Device** argDevice) const
{
    IFRNULL(argDevice);
...
ID2D1Device* D2DContext::GetD2DDeviceNoRef(void) const
{
    return this->mD2DDevice.get();
}


HRESULT D2DContext::GetD2DFactory(ID2D1Factory1** argFactory) const
{
    IFRNULL(argFactory);
...
ID2D1Factory1* D2DContext::GetD2DFactoryNoRef(void) const
{
    return this->mD2DFactory.get();
}


D3D_FEATURE_LEVEL D2DContext::GetD2DFeatureLevel(void) const
{
    return this->mD2DFeatureLevel;
}


D3D_FEATURE_LEVEL D2DContext::GetHighestFeatureLevel(void) const
{
    return this->mD3DFeatureLevels[mHighestFeatureLevelIndex];
}


D3D_FEATURE_LEVEL D2DContext::GetLowestFeatureLevel(void) const
{
...
HRESULT D2DContext::CreateFrame(const D2D1_SIZE_U& argFrameSize, UINT argFrameIndex, ID2D1Bitmap1* argAtlas, ID2D1Bitmap1** argFrame, D2D1_BITMAP_OPTIONS argOptions)
{
    HRESULT hResult;
...
HRESULT D2DContext::LoadImageFromFile(wchar_t const * const argFileName, ID2D1Bitmap1** argBitmapOut)
{
    HRESULT hResult;
...
HRESULT D2DContext::RenderOutput(ID2D1Image* argEffectOutput)
{
    HRESULT hResult;
    ...
    IFC(this->mD2DDeviceContext->CreateBitmap(
        D2D1::SizeU(1, 1),
        nullptr,
        0,
...
HRESULT D2DContext::SaveImageToFile(wchar_t const * const argFileName, ID2D1Image* argImage, ImageContainerFormat argFormat)
{
    HRESULT hResult;
    ...
    if (0 == ::wcslen(argFileName))
    {
        return E_INVALIDARG;
    }

    // This function object returns a file name with matching extension
    auto GetFileName = [](wchar_t const * const argFileName, wchar_t const * const * const argExtensions, UINT argLength)->std::unique_ptr<wchar_t[]>
    {
        // This function object appends an extension to a file name
        auto AppendExtension = [](wchar_t const * const argFileName, wchar_t const * const argExtension)->std::unique_ptr<wchar_t[]>
        {
            if (nullptr == argFileName)
            {
                return nullptr;
            }

            if (nullptr == argExtension)
            {
                return nullptr;
            }

            auto fileNameLength = ::wcslen(argFileName);
            auto extensionLength = ::wcslen(argExtension);
            auto stringLength = fileNameLength + extensionLength;
            auto stringSize = stringLength + 1;

            std::unique_ptr<wchar_t[]> string(new wchar_t[stringSize]);

            ::wcscpy(string.get(), argFileName);
            ::wcscpy(string.get() + fileNameLength, argExtension);

            return string;
        };

        // This function object returns true if a given file name has the given extension at the end
        auto FileNameHasExtension = [](wchar_t const * const argFileName, wchar_t const * const argExtension)->bool
        {
            if (nullptr == argFileName)
            {
                return false;
            ...
            auto compare = ::_wcsicmp(argFileName + fileNameLength - extensionLength, argExtension);
        ...
        std::unique_ptr<wchar_t[]> fileNameWithExtension;
        bool hasExtension = false;

        for (UINT i = 0; i < argLength; ++i)
        {
            if (FileNameHasExtension(argFileName, argExtensions[i]))
            {
                hasExtension = true;
                break;
            }
        ...
        {
            auto fileNameLength = ::wcslen(argFileName);
            fileNameWithExtension = std::unique_ptr<wchar_t[]>(new wchar_t[fileNameLength + 1]);
            ::wcscpy(fileNameWithExtension.get(), argFileName);
        }
        else
        {
            fileNameWithExtension = AppendExtension(argFileName, argExtensions[0]);
        }
    ...
    switch (argFormat)
    {
        case ImageContainerFormat::Bmp:
        {
            const wchar_t* extensions[] = { L".bmp" };
            fileNameWithExtension = GetFileName(argFileName, extensions, ARRAYSIZE(extensions));
        ...
        {
            const wchar_t* extensions[] = { L".png" };
            fileNameWithExtension = GetFileName(argFileName, extensions, ARRAYSIZE(extensions));
        ...
        case ImageContainerFormat::Ico:
        {
            const wchar_t* extensions[] = { L".ico" };
            fileNameWithExtension = GetFileName(argFileName, extensions, ARRAYSIZE(extensions));
        ...
        {
            const wchar_t* extensions[] = { L".jpeg", L".jpg", L".jpe", L".jfif" };
            fileNameWithExtension = GetFileName(argFileName, extensions, ARRAYSIZE(extensions));
        ...
        {
            const wchar_t* extensions[] = { L".tiff", L".tif" };
            fileNameWithExtension = GetFileName(argFileName, extensions, ARRAYSIZE(extensions));
        ...
        {
            const wchar_t* extensions[] = { L".gif" };
            fileNameWithExtension = GetFileName(argFileName, extensions, ARRAYSIZE(extensions));
        ...
        case ImageContainerFormat::Wmp:
        {
            const wchar_t* extensions[] = { L".wmp" };
            fileNameWithExtension = GetFileName(argFileName, extensions, ARRAYSIZE(extensions));
    ...
    IFC(targetStream->InitializeFromFilename(fileNameWithExtension.get(), GENERIC_WRITE));
    ...
    IFC(imagingFactory2->CreateEncoder(containerFormat, nullptr, &bitmapEncoder));
