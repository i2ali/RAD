...
{
    VERIFY_SUCCEEDED(::CreateSlicerFactory(&this->mSlicerFactory, RAD::ComputeShaderModel::CS_4_0));
    VERIFY_SUCCEEDED(this->mSlicerFactory->GetD2DContext(&this->mD2DContext));
    VERIFY_SUCCEEDED(::RADRegisterEffects(this->mD2DContext->GetD2DFactoryNoRef()));
...
bool Effects::ClassCleanup(void)
{
    ...
    {
        ::RADUnregisterEffects(this->mD2DContext->GetD2DFactoryNoRef());
    }
...
void Effects::LabConversion(void)
{
    ...
    VERIFY_SUCCEEDED(this->mD2DContext->GetD2DDeviceNoRef()->CreateDeviceContext(D2D1_DEVICE_CONTEXT_OPTIONS_NONE, &deviceContext));

    // Generate the test image
    CComPtr<ID2D1Bitmap1> image;
    this->RenderRects(deviceContext, imageSize, rects, &image);
    VERIFY_SUCCEEDED(this->mD2DContext->SaveImageToFile(L"LabConversion_Image", image, RAD::ImageContainerFormat::Bmp));

    // Create the effects
    CComPtr<ID2D1Effect> rgbToLab;
    CComPtr<ID2D1Effect> debugImageData;

    VERIFY_SUCCEEDED(deviceContext->CreateEffect(CLSID_RADRgbToLab, &rgbToLab));
    VERIFY_SUCCEEDED(deviceContext->CreateEffect(CLSID_RADDebugImageData, &debugImageData));
    ...
    VERIFY_SUCCEEDED(this->mD2DContext->RenderOutput(outputOfDebugImageData));
    ...
    VERIFY_SUCCEEDED(debugImageData->GetValue(RAD_DEBUG_IMAGE_DATA_PROP_IMAGE_DATA, &imageData));
    VERIFY_SUCCEEDED(debugImageData->GetValue(RAD_DEBUG_IMAGE_DATA_PROP_IMAGE_DATA_LENGTH, &imageDataLength));
...
void Effects::LabDistance(void)
{
    ...
    VERIFY_SUCCEEDED(this->mD2DContext->SaveImageToFile(L"LabDistance_Image1", image1, RAD::ImageContainerFormat::Bmp));
    ...
    VERIFY_SUCCEEDED(this->mD2DContext->SaveImageToFile(L"LabDistance_Image2", image2, RAD::ImageContainerFormat::Bmp));
    ...
    VERIFY_SUCCEEDED(deviceContext->CreateEffect(CLSID_RADRgbToLab, &rgbToLab1));
    VERIFY_SUCCEEDED(deviceContext->CreateEffect(CLSID_RADRgbToLab, &rgbToLab2));
    VERIFY_SUCCEEDED(deviceContext->CreateEffect(CLSID_RADLabImageComparer, &labImageComparer));
    VERIFY_SUCCEEDED(deviceContext->CreateEffect(CLSID_RADDebugImageData, &debugImageData));
    ...
    VERIFY_SUCCEEDED(deviceContext->CreateEffect(CLSID_RADCheckColorDistance, &checkColorDistance));

    checkColorDistance->SetInput(0, outputOfLabImageComparer);
    checkColorDistance->GetOutput(&outputOfCheckColorDistance);

    VERIFY_SUCCEEDED(this->mD2DContext->RenderOutput(outputOfCheckColorDistance));
    ...
    VERIFY_SUCCEEDED(checkColorDistance->GetValue(RAD_CHECK_COLOR_DISTANCE_PROP_PIXEL_MATCH_COUNT, &pixelMatchCount));
    VERIFY_SUCCEEDED(checkColorDistance->GetValue(RAD_CHECK_COLOR_DISTANCE_PROP_PIXEL_MATCH_FIRST_X, &pixelMatchFirstX));
    VERIFY_SUCCEEDED(checkColorDistance->GetValue(RAD_CHECK_COLOR_DISTANCE_PROP_PIXEL_MATCH_FIRST_Y, &pixelMatchFirstY));
    VERIFY_SUCCEEDED(checkColorDistance->GetValue(RAD_CHECK_COLOR_DISTANCE_PROP_PIXEL_MISMATCH_COUNT, &pixelMismatchCount));
    VERIFY_SUCCEEDED(checkColorDistance->GetValue(RAD_CHECK_COLOR_DISTANCE_PROP_PIXEL_MISMATCH_FIRST_X, &pixelMismatchFirstX));
    VERIFY_SUCCEEDED(checkColorDistance->GetValue(RAD_CHECK_COLOR_DISTANCE_PROP_PIXEL_MISMATCH_FIRST_Y, &pixelMismatchFirstY));
...
void Effects::LabColorComparerGraph(void)
{
    ...
    CComPtr<ID2D1Bitmap1> image;
    this->RenderRects(deviceContext, imageSize, rects, &image);
    VERIFY_SUCCEEDED(this->mD2DContext->SaveImageToFile(L"LabColorComparerGraph_Image", image, RAD::ImageContainerFormat::Bmp));
    ...
    VERIFY_SUCCEEDED(deviceContext->CreateEffect(CLSID_RADRgbToLab, &rgbToLab));
    VERIFY_SUCCEEDED(deviceContext->CreateEffect(CLSID_RADLabColorComparer, &labColorComparer));
    VERIFY_SUCCEEDED(deviceContext->CreateEffect(CLSID_RADColorDistanceToRgb, &colorDistanceToRgb));
    VERIFY_SUCCEEDED(deviceContext->CreateEffect(CLSID_RADCheckColorDistance, &checkColorDistance));
    ...
    VERIFY_SUCCEEDED(labColorComparer->SetValue(RAD_LAB_COLOR_COMPARER_PROP_L, static_cast<FLOAT>(inputLabColor.GetL())));
    VERIFY_SUCCEEDED(labColorComparer->SetValue(RAD_LAB_COLOR_COMPARER_PROP_A, static_cast<FLOAT>(inputLabColor.GetA())));
    VERIFY_SUCCEEDED(labColorComparer->SetValue(RAD_LAB_COLOR_COMPARER_PROP_B, static_cast<FLOAT>(inputLabColor.GetB())));
    ...
    VERIFY_SUCCEEDED(this->mD2DContext->RenderOutput(outputOfColorDistanceToRgb));
    VERIFY_SUCCEEDED(this->mD2DContext->RenderOutput(outputOfCheckColorDistance));

    VERIFY_SUCCEEDED(this->mD2DContext->SaveImageToFile(L"LabColorComparerGraph_outputOfRgbToLab", outputOfRgbToLab, RAD::ImageContainerFormat::Bmp));
    VERIFY_SUCCEEDED(this->mD2DContext->SaveImageToFile(L"LabColorComparerGraph_outputOfLabColorComparer", outputOfLabColorComparer, RAD::ImageContainerFormat::Bmp));
    VERIFY_SUCCEEDED(this->mD2DContext->SaveImageToFile(L"LabColorComparerGraph_outputOfColorDistanceToRgb", outputOfColorDistanceToRgb, RAD::ImageContainerFormat::Bmp));
...
void Effects::LabImageComparerGraph(void)
{
    ...
    this->RenderRects(deviceContext, imageSize, rects1, &image1);
    VERIFY_SUCCEEDED(this->mD2DContext->SaveImageToFile(L"LabImageComparerGraph_Image1", image1, RAD::ImageContainerFormat::Bmp));
    ...
    this->RenderRects(deviceContext, imageSize, rects2, &image2);
    VERIFY_SUCCEEDED(this->mD2DContext->SaveImageToFile(L"LabImageComparerGraph_Image2", image2, RAD::ImageContainerFormat::Bmp));
    ...
    VERIFY_SUCCEEDED(this->mD2DContext->SaveImageToFile(L"LabImageComparerGraph_outputOfRgbToLab1", outputOfRgbToLab1, RAD::ImageContainerFormat::Bmp));
    VERIFY_SUCCEEDED(this->mD2DContext->SaveImageToFile(L"LabImageComparerGraph_outputOfRgbToLab2", outputOfRgbToLab2, RAD::ImageContainerFormat::Bmp));
    VERIFY_SUCCEEDED(this->mD2DContext->SaveImageToFile(L"LabImageComparerGraph_outputOfLabImageComparer", outputOfLabImageComparer, RAD::ImageContainerFormat::Bmp));
    VERIFY_SUCCEEDED(this->mD2DContext->SaveImageToFile(L"LabImageComparerGraph_outputOfColorDistanceToRgb", outputOfColorDistanceToRgb, RAD::ImageContainerFormat::Bmp));
...
void Effects::RgbColorComparerGraph(void)
{
    ...
    VERIFY_SUCCEEDED(this->mD2DContext->SaveImageToFile(L"RgbColorComparerGraph_Image", image, RAD::ImageContainerFormat::Bmp));

    // Create the effects
    CComPtr<ID2D1Effect> rgbColorComparer;
    VERIFY_SUCCEEDED(deviceContext->CreateEffect(CLSID_RADRgbColorComparer, &rgbColorComparer));

    // Create the effect graph
    CComPtr<ID2D1Image> outputOfRgbColorComparer;

    rgbColorComparer->SetValue(RAD_RGB_COLOR_COMPARER_PROP_RED,   static_cast<float>(inputColor.GetRed()));
    rgbColorComparer->SetValue(RAD_RGB_COLOR_COMPARER_PROP_GREEN, static_cast<float>(inputColor.GetGreen()));
    rgbColorComparer->SetValue(RAD_RGB_COLOR_COMPARER_PROP_BLUE,  static_cast<float>(inputColor.GetBlue()));

    rgbColorComparer->SetInput(0, image);
    rgbColorComparer->GetOutput(&outputOfRgbColorComparer);

    VERIFY_SUCCEEDED(this->mD2DContext->RenderOutput(outputOfRgbColorComparer));
    ...
    VERIFY_SUCCEEDED(rgbColorComparer->GetValue(RAD_RGB_COLOR_COMPARER_PROP_PIXEL_MATCH_COUNT, &pixelMatchCount));
    VERIFY_SUCCEEDED(rgbColorComparer->GetValue(RAD_RGB_COLOR_COMPARER_PROP_PIXEL_MATCH_FIRST_X, &pixelMatchFirstX));
    VERIFY_SUCCEEDED(rgbColorComparer->GetValue(RAD_RGB_COLOR_COMPARER_PROP_PIXEL_MATCH_FIRST_Y, &pixelMatchFirstY));
    VERIFY_SUCCEEDED(rgbColorComparer->GetValue(RAD_RGB_COLOR_COMPARER_PROP_PIXEL_MISMATCH_COUNT, &pixelMismatchCount));
    VERIFY_SUCCEEDED(rgbColorComparer->GetValue(RAD_RGB_COLOR_COMPARER_PROP_PIXEL_MISMATCH_FIRST_X, &pixelMismatchFirstX));
    VERIFY_SUCCEEDED(rgbColorComparer->GetValue(RAD_RGB_COLOR_COMPARER_PROP_PIXEL_MISMATCH_FIRST_Y, &pixelMismatchFirstY));
...
void Effects::RgbImageComparerGraph(void)
{
    ...
    VERIFY_SUCCEEDED(this->mD2DContext->SaveImageToFile(L"RgbImageComparerGraph_Image1", image1, RAD::ImageContainerFormat::Bmp));
    ...
    VERIFY_SUCCEEDED(this->mD2DContext->SaveImageToFile(L"RgbImageComparerGraph_Image2", image2, RAD::ImageContainerFormat::Bmp));


    // Create the effects
    CComPtr<ID2D1Effect> rgbImageComparer;
    VERIFY_SUCCEEDED(deviceContext->CreateEffect(CLSID_RADRgbImageComparer, &rgbImageComparer));
    ...
    VERIFY_SUCCEEDED(this->mD2DContext->RenderOutput(outputOfRgbImageComparer));
    ...
    VERIFY_SUCCEEDED(rgbImageComparer->GetValue(RAD_RGB_IMAGE_COMPARER_PROP_PIXEL_MATCH_COUNT, &pixelMatchCount));
    VERIFY_SUCCEEDED(rgbImageComparer->GetValue(RAD_RGB_IMAGE_COMPARER_PROP_PIXEL_MATCH_FIRST_X, &pixelMatchFirstX));
    VERIFY_SUCCEEDED(rgbImageComparer->GetValue(RAD_RGB_IMAGE_COMPARER_PROP_PIXEL_MATCH_FIRST_Y, &pixelMatchFirstY));
    VERIFY_SUCCEEDED(rgbImageComparer->GetValue(RAD_RGB_IMAGE_COMPARER_PROP_PIXEL_MISMATCH_COUNT, &pixelMismatchCount));
    VERIFY_SUCCEEDED(rgbImageComparer->GetValue(RAD_RGB_IMAGE_COMPARER_PROP_PIXEL_MISMATCH_FIRST_X, &pixelMismatchFirstX));
    VERIFY_SUCCEEDED(rgbImageComparer->GetValue(RAD_RGB_IMAGE_COMPARER_PROP_PIXEL_MISMATCH_FIRST_Y, &pixelMismatchFirstY));
