...
void DwmSlicerBoundaryCases::CaptureOnTheCornersOfThePrimaryDisplay(void)
{
    ...
    for (int current = 0; current < ARRAYSIZE(rects); ++current)
    {
        // Non-stereo
        {
            ...
            VERIFY_SUCCEEDED(this->mSlicer->StartCapture(rects[current], 1, 0, RAD::Channel::Left, &numberOfFramesThatCanBeCaptured));
        ...
        {
            UINT numberOfFramesThatCanBeCaptured = 0;
            VERIFY_SUCCEEDED(this->mSlicer->StartCapture(rects[current], 1, 0, RAD::Channel::LeftAndRight, &numberOfFramesThatCanBeCaptured));
            Log::Comment(String().Format(L"Number of frames that can be captured: %d", numberOfFramesThatCanBeCaptured));
...
void DwmSlicerBoundaryCases::CaptureOneByOneOutside(void)
{
    ...
    for (int current = 0; current < ARRAYSIZE(rects); ++current)
    {
        ...
        {
            UINT numberOfFramesThatCanBeCaptured = 0;
            VERIFY_FAILED(this->mSlicer->StartCapture(rects[current], 1, 0, RAD::Channel::Left, &numberOfFramesThatCanBeCaptured));
        }

        // Stereo
        {
            UINT numberOfFramesThatCanBeCaptured = 0;
            VERIFY_FAILED(this->mSlicer->StartCapture(rects[current], 1, 0, RAD::Channel::LeftAndRight, &numberOfFramesThatCanBeCaptured));
        }
    }
}
